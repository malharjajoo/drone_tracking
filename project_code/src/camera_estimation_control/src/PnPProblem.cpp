...
// For some reason the distortion parameters dont work. Hence using zeros as of now...
PnPProblem::PnPProblem(const double params[], const double distortioncoeffs[])
{

  _D_matrix = cv::Mat::zeros(4, 1, CV_64FC1); 
 // _D_matrix.at<double>(0) = distortioncoeffs[0];     
 // _D_matrix.at<double>(1) = distortioncoeffs[1];       // Radial and tangential 
 // _D_matrix.at<double>(2) = distortioncoeffs[2];       // distortion parameters
 // _D_matrix.at<double>(3) = distortioncoeffs[3];
                               ...
                               int flags)
{
  ...
  // Pose estimation
  //CV_ITERATIVE
  bool correspondence = cv::solvePnP( list_points3d, list_points2d, _A_matrix, cv::Mat(), rvec, tvec,
                                          false/*useExtrinsicGuess*/, CV_EPNP);
